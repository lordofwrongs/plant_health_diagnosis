...
| Edge functions | Deno (Supabase) | `plant-processor` (full AI pipeline), `plant-chat` (Q&A), `care-reminder` (hourly: watering reminders + pest follow-up reminders), `weekly-digest` (weekly email via Brevo), `support-request` (in-app support form → Brevo email to botaniqsupport@gmail.com) |
...
| `src/components/SupportModal.jsx` | In-app support form — name/email/message, POSTs to `support-request` edge function, confirmation screen with optional self-copy mailto |
| `supabase/functions/support-request/index.ts` | Receives support form POST, sends HTML email to botaniqsupport@gmail.com via Brevo with Reply-To set to user's email |
...
plant_logs
  ...
  plantnet_reference_image text -- PlantNet reference leaf image URL (medium size) — ✅ added Sprint 23, migration executed
  nutrient_recommendations jsonb -- {deficiency_detected, deficiency_signs, primary_fix, organic_option, diy_option, stage_note, caution} — ✅ added Sprint 23, migration executed
  harvest_guide jsonb           -- {days_to_first_harvest, current_stage_estimate, visual_readiness_cues, check_frequency, how_to_harvest, post_harvest_tip, important_warning} — ✅ added Sprint 23, migration executed
  plant_overview text           -- AI-generated 1–2 sentence natural language plant description (type + common use) — ✅ added Sprint 24, migration executed
...
| 22 | **In-app support form**: `?` button in nav opens `SupportModal` — name (optional), email (required), message (required, 1000 char limit). POSTs to new `support-request` Supabase edge function which sends a formatted HTML email to botaniqsupport@gmail.com via Brevo with Reply-To set to the user's email. Confirmation screen shows all submitted details. Optional "Email myself a copy" button opens pre-filled mailto to user's own address. Error boundary fallback retains mailto link for crash scenarios. ✅ `support-request` edge function deployed. ✅ Frontend deployed via git push → Vercel. |
| 21 | **Pest follow-up delivery + cross-platform baseline** (audit fix + UX): (1) **Pest follow-up notifications delivered** — `care-reminder` now reads `follow_up_reminders` (step 8) after each watering pass; sends push notification per subscribed user for every due row; marks `processed = true` regardless of subscription status to prevent infinite retry for users who never opted in; stale endpoints from this pass cleaned with the main batch. Response now includes `followUpSent` count. (2) **iOS safe area on bottom sheet** — `PhotoSourceSheet` bottom padding changed from `40px` to `max(40px, calc(env(safe-area-inset-bottom) + 20px))` so Cancel button clears the iPhone home indicator. (3) **Viewport height fix** — `#root` in `index.css` now has `min-height: 100dvh` as progressive enhancement above `100vh`; `appContainer` in `App.jsx` uses `100svh` to prevent iOS Safari address-bar overflow clipping. ✅ `care-reminder` redeployed. ✅ Frontend deployed via git push → Vercel. |
| 24 | **Plant overview blurb in About tab**: Gemini instruction 19 generates a 1–2 sentence natural language description of each plant (category + everyday use) stored in `plant_logs.plant_overview`. Displayed as "About This Plant" card at the top of the About tab in `ResultsScreen`. Null-guarded — card hidden for pre-Sprint-24 records. **DB migration**: `sprint24_plant_overview.sql` — adds `plant_overview text` column. ✅ Migration executed. ✅ `plant-processor` redeployed. ✅ Frontend deployed via git push → Vercel. |
| 23 | **Results page overhaul — 3-tab layout, reference image, nutrient & harvest guidance**: (1) **3-tab layout** — `ResultsScreen` restructured from 14-card linear scroll into tabbed layout: **Diagnosis** (hero, weather alert, visual analysis, vital signs, growth journey, pest card) / **Care** (care schedule, nutrient recommendations, harvest guide, reminder nudge) / **About** (classification, toxicity, environment). Feedback widget and Q&A remain below tabs always. (2) **ReferenceImagePanel** — when PlantNet returns a reference image and `AccuracyScore < 90`, shows a compact leaf comparison card between hero and tabs with "Does your plant's leaf shape match this?" prompt + wrong plant CTA. PlantNet URL param changed from `include-related-images=false` to `true`; reference image URL extracted from `results[0].images[0].url.m` and stored in `plant_logs.plantnet_reference_image`. Correction re-run preserves prior reference image from DB. (3) **NutrientCard** — Gemini instruction 16 produces `nutrient_recommendations` jsonb: deficiency label, deficiency signs, primary fix (product/recipe/application), organic alternative, DIY recipe, stage note, caution. Null when `vital_signs.nutrients ≥ 75` and no visible deficiency. Shown in Care tab. (4) **HarvestGuideCard** — Gemini instruction 17 produces `harvest_guide` jsonb for edible plants: days to first harvest, current stage estimate, visual readiness cues (✓ list), check frequency, how-to-harvest, post-harvest tip, important warning. Null for non-edible plants. Shown in Care tab when `plant_classification.is_edible === true`. **DB migration**: `sprint23_results_overhaul.sql` — adds `plantnet_reference_image text`, `nutrient_recommendations jsonb`, `harvest_guide jsonb` to `plant_logs`. ✅ Migration executed. ✅ `plant-processor` redeployed. ✅ Frontend deployed via git push → Vercel. |
...
| **UX De-congestion (Results Tabs)** | ✅ Done (Sprint 23) — `ResultsScreen` restructured into 3-tab layout (Diagnosis / Care / About); 14-card scroll replaced with organised tab bodies. | High: Dramatically reduces scroll fatigue. |
...
# support-request: --no-verify-jwt (called from frontend, no Supabase JWT; guests can submit too)
